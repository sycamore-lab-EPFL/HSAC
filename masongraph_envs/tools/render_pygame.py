import pygame
import numpy as np
from simulator.hexsimulator.blocks.blocks_norot import Grid,DiscreteBlockNorot as Block, side2corner, outer_sides,get_cm
...
def open_window(gridsize,scale):
    ...
    size = ((gridsize[0]+gridsize[1]/2+0.5)*scale,(gridsize[1]+1)*scale*np.sqrt(3)/2)
    pygame.init()
    pygame.display.init()
...
def fig2surface(fig,gridsize,scale=1):
    fig.canvas.draw()
    #image_flat = np.frombuffer(fig.canvas.tostring_rgb(), dtype='uint8')  # (H * W * 3,)
    # NOTE: reversed converts (W, H) from get_width_height to (H, W)
    image =  pygame.image.frombytes(fig.canvas.tostring_argb(),
                                    (fig.get_size_inches()*fig.dpi).astype(int),'ARGB')
