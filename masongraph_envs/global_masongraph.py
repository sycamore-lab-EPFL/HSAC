...
class GlobalMasonGraphCover():
    ...
    def action_metadata(self):
        if self.vertical:
            return {'discrete':[len(self.block_type),1],
                    'continuous':4,
                    'boundary':np.array([[-self.max_dist/2,self.max_dist/2],
                                         [-self.max_dist/2,self.max_dist/2],
                                         [-2,2],
                                         [-2,2]])}
        else:
            return {'discrete':[len(self.block_type),1],
    ...
    def step(self,action,render=False,normalized=True):
        ...
        if isinstance(action,tuple):
            action = converter.tuple2dict_global(action,len(self.block_type),vertical=True,normalized=normalized,action_boundary=self.action_metadata()['boundary'])
            info['rotation_offset'] =  geometry.rotmat_from_2D(action['rotation_offset'])[:,:2].reshape(-1,2)
        ...
        if env_ids.shape[0] > 0:
            valid_a,rewards[env_ids],info['action'] = self.sim.put_block_from(self.block_type[action['block_type']].squeeze(),
                                                                              action['pos'],
                                                                              action['direction'],
                                                                              init_rot = action['rotation_offset'],
                                                                                env_ids=env_ids,
                                                                                touch_target=False,
                                                                                hold = False,
                                                                                target = -1,
