...
if __name__ == "__main__":
    ...
    print("Start test")
    n_batch = 1
    env = Env(n_batch=n_batch,min_area=[3,5],max_area=[3.1,5.1],scales=[1],autoreset=False,
              max_blocks=30,friction_coef=0.5,max_dist = 40,tol=1e-2,max_action_substeps=100,reset_on_fall=False,rep='bframegraph')
    env.reset(seed=seed)
    activeUI(env.sim)
    ...
    for i in range(20):
        ...
        rot = np.array([[0,1]]*n_batch)
        action2 = {"robot_id":np.full(n_batch,i%env.n_robots,dtype=int),
                "block_type":np.zeros(n_batch,dtype=int),
                "support_block":np.ones(n_batch,dtype=int)*2*i,
                "center_offset":np.tile([0.62,0,0],(n_batch,1))*((i%2)*2-1),
                'rotation_offset':rot,
                "direction":direction,
                }
