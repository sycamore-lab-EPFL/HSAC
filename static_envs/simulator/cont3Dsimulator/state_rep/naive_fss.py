from .base_fixed_size_set import BaseSetConstructor,ISACTION_FEATURE,ISOBJECTIVE_FEATURE,ISBLOCK_FEATURE
import torch
import numpy as np
...
class NaiveSetConstructor(BaseSetConstructor):
    ...
    def embed_blocks(self, blocks, contacts, is_ground, is_held, objectives,referencial=None,**kwargs):
        ...
        embedding[:,:,1]=torch.tensor(is_ground)
        embedding[:,:,2]=torch.tensor(is_held)
        embedding[:,:,3:6]=torch.tensor(np.array([b.center_mass if b is not None else [0,0,0] for b in blocks.flat])).view(blocks.shape+(3,))
        embedding[:,:,6:15]=torch.tensor(referencial).flatten(2)
        return embedding
