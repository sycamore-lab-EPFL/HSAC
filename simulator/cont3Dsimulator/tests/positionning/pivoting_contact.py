...
sim = sim3D.BatchSimulator(n_batch=1,tol=1e-2,max_dist=3,mu=2,max_action_steps=20)
sim3D.activeUI(sim)
sim.prepare_action()
...
for i in range(1,2):
    ...
    #valid = sim.put_block_from([objs[i]],[objs[i].centroid+[0.,-1.1,1.7]],force = [np.array([0,1,-2])],slide=False)
    print(valid)
sim.render()
ps.show()