...
sim = sim3D.BatchSimulator(n_batch=1,tol=1e-2,max_dist=3,max_action_steps=100,mu=15,)
sim3D.activeUI(sim)
sim.prepare_action()
sim.put_block_abs([objs[5].apply_transform(trimesh.transformations.rotation_matrix(-1.7,[1,0,0.1],point=objs[5].vertices[0]))],[objs[0].centroid])
#valid = sim.put_block_abs([objs[5]],[objs[0].centroid+[0,-0.3,0.15]])
#print(valid)
for i in range(1,2):
    sim.prepare_action()
    valid = sim.put_block_from([objs[i]],[objs[i].centroid+[0.0,1.1,2]],force = [np.array([0,-1,-2])],slide=True,pivot=True)
    print(valid)
sim.render()
ps.show()