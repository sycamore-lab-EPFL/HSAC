...
ISBLOCK_FEATURE = 0
ISACTION_FEATURE = 1
ISOBJECTIVE_FEATURE = 2
class BaseSetConstructor:
    ...
    def compute_full_graph_batched(self,nblocks,blocks,contacts,is_ground,is_held,objectives,referencial = None, **kwargs):
        ...
        actionnodes = self.embed_actions(None, blocks, contacts, is_ground, is_held, objectives,**kwargs)
        data = torch.concatenate([blocknodes, objectivenodes, actionnodes], dim=1)
    
        assert data.shape[1] <= self.n_nodes
        #pad with zeros if number of nodes is less than n_nodes
        mask = torch.zeros((data.shape[0], self.n_nodes), device=device, dtype=torch.bool)
        mask[:,:data.shape[1]]=True
        data = torch.concatenate([data, torch.zeros((data.shape[0], self.n_nodes - data.shape[1], self.embedding_dim), device=device, dtype=self.floatType)], dim=1)
        

        assert data.shape[2] == self.embedding_dim
        return (data,mask)
    @property
    def metadata(self):
